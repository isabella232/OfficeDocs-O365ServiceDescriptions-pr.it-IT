---
title: Descrizione servizio Yammer
ms.author: sharik
author: skjerland
manager: mnirkhe
audience: ITPro
ms.topic: reference
f1_keywords:
- yammer-service-description
ms.service: o365-administration
localization_priority: Normal
ms.custom: Adm_ServiceDesc
ms.assetid: 268aadc2-8916-4f9a-9e37-9e509b52d27d
description: Yammer è un social network aziendale efficace, sicuro e privato. Yammer consente ai dipendenti di rafforzare e migliorare la loro produttività consentendo di collaborare facilmente, prendere decisioni più velocemente e organizzarsi autonomamente in team per affrontare qualunque tipo di sfida aziendale. Si tratta di un nuovo modo di lavorare che guida in maniera naturale l'allineamento e l'agibilità aziendale, riduce la durata dei cicli, coinvolge i dipendenti e migliora le relazioni tra clienti e partner.
ms.openlocfilehash: 4140f3cb779c6b444dfff9626f8fadb1686e2ddd
ms.sourcegitcommit: 96dc758c790ddaf05f5c2b836451b417729cf119
ms.translationtype: MT
ms.contentlocale: it-IT
ms.lasthandoff: 07/18/2019
ms.locfileid: "35777887"
---
# <a name="yammer-service-description"></a>Descrizione servizio Yammer

Yammer è un social network aziendale efficace, sicuro e privato. Yammer consente ai dipendenti di rafforzare e migliorare la loro produttività consentendo di collaborare facilmente, prendere decisioni più velocemente e organizzarsi autonomamente in team per affrontare qualunque tipo di sfida aziendale. Si tratta di un nuovo modo di lavorare che guida in maniera naturale l'allineamento e l'agibilità aziendale, riduce la durata dei cicli, coinvolge i dipendenti e migliora le relazioni tra clienti e partner.
  
Per un elenco dei piani di Office 365 che includono Yammer, vedere [Famiglie e piani di servizi di Office 365](../office-365-platform-service-description/office-365-plan-options.md#office-365-service-families-and-plans).
  
Per un confronto tra le funzionalità offerte dai vari piani, vedere la pagina relativa al [confronto tra i piani di Office 365 for business](http://go.microsoft.com/fwlink/?LinkID=799177&amp;clcid=0x409).
  
> [!TIP]
> È possibile esportare, salvare e stampare le pagine della descrizione del servizio Office 365. Ulteriori informazioni su come [esportare più pagine](https://go.microsoft.com/fwlink/?LinkId=403349). 
  
## <a name="system-requirements"></a>Requisiti di sistema

Per i requisiti di sistema per Office 365, il servizio a sottoscrizione mensile per aziende, centri di istruzione ed enti pubblici, vedere [Requisiti di sistema di Office 365](https://products.office.com/office-system-requirements/#Office365forBEG).
  
## <a name="yammer-plans"></a>Piani Yammer

Yammer è disponibile attraverso i piani di sottoscrizione elencati di seguito:
  
|**Piano**|**Descrizione**|
|:-----|:-----|
|[Yammer Basic](https://go.microsoft.com/fwlink/?LinkId=691112) <br/> |Social network aziendale - sempre e ovunque.  <br/> |
|[Yammer Enterprise](https://go.microsoft.com/fwlink/?LinkId=691122) <br/> |Social network aziendale con sistemi di sicurezza, integrazione e amministrazione migliorati.  <br/> |
   
> [!NOTE]
> Le funzionalità e i criteri descritti in [Descrizione dei servizi della piattaforma Office 365](../office-365-platform-service-description/office-365-platform-service-description.md) non vengono applicati al servizio Yammer. 
  
## <a name="yammer-enterprise-service-communications-policy"></a>Criteri di comunicazione del servizio Yammer Enterprise
<a name="YammerCommsPolicy"> </a>

Yammer è una parte di Office 365 e si attiene ai [criteri di gestione delle modifiche di office 365](https://blogs.office.com/2015/05/05/manage-change-and-stay-informed-in-office-365/), ma non utilizza la versione di destinazione per distribuire gli aggiornamenti. 
  
Tutti gli utenti di Yammer sono soggetti a un test delle funzionalità di tipo A/B. In questo modo Microsoft è in grado di verificare le funzionalità per il miglioramento dell'esperienza, delle prestazioni e della stabilità in molti ambienti e dispositivi. Gli utenti all'interno della stessa rete possono avere diverse esperienze a causa dei test. Non è possibile rifiutarsi di essere sottoposti al test A/B.
  
Eventuali modifiche (test A/B, funzionalità nuove e deprecate, limiti di conformità, modifiche relative al flusso di lavoro) che richiedono l'intervento dei clienti o che dovrebbero influire in modo significativo sulla loro esperienza verranno comunicate in anticipo. Abbiamo identificato uno scenario in cui Yammer apporta una modifica che riguarda un sottoinsieme limitato di utenti. Si pensi, ad esempio, alla modifica o alla rimozione di una funzionalità, come l'estensione Chrome di Yammer, che interessa un numero limitato di utenti che l'hanno abilitata. In questi casi, oltre a inviare la solita notifica agli amministratori verificati (come indicato nel criterio citato in precedenza), informeremo direttamente gli utenti interessati dalla modifica per garantire loro buone prestazioni. Per modifiche suddivise in categorie come "Modifiche che richiedono l'azione (Pianificazione delle modifiche)" o "Modifiche che richiedono la consapevolezza (Rimanere informati)" che riguardano solo un determinato insieme di utenti, si possono informare direttamente tali utenti. La tabella seguente è stata aggiornata affinché presenti questo cambiamento. 
  
I canali usati per le comunicazioni ai clienti dipenderanno dall'impatto:
  
|||
|:-----|:-----|
|**Modifiche che richiedono l'azione (Pianificazione delle modifiche)**         All'amministratore viene richiesto di apportare modifiche per mantenere il servizio in esecuzione, aggiornamenti che possono influire sulla conformità dell'organizzazione  <br/> |Centro messaggi di Office 365 Consente di inviare messaggi di posta elettronica direttamente agli amministratori verificati di Yammer Guida di orientamento di Office 365Gruppo esterno sugli aggiornamenti dei servizi YammerIn caso di modifiche che interessano un determinato gruppo di utenti, possiamo anche inviare delle notifiche direttamente a tali utenti. |
|**Modifiche che richiedono consapevolezza (Rimanere informati)**         Modifiche che non richiedono l'intervento dell'amministratore, modifiche alla progettazione visiva  <br/> |Centro messaggi di Office 365 Guida di orientamento di Office 365Gruppo esterno sugli aggiornamenti dei servizi YammerIn caso di modifiche che interessano un determinato gruppo di utenti, possiamo anche inviare delle notifiche direttamente a tali utenti. |
|**Non critiche**         Modifiche che non sono critiche e che non richiedono l'azione o la conoscenza degli utenti, anche se possono essere riscontrate da alcuni utenti  <br/> |In genere non verranno comunicate. |
   
## <a name="feature-availability-across-yammer-plans"></a>Disponibilità delle funzionalità tra i piani di Yammer
<a name="YammerCommsPolicy"> </a>

Se si ha bisogno di assistenza per capire quale piano Yammer meglio si adatta alle esigenze della propria organizzazione, utilizzare la seguente tabella per vedere le funzionalità disponibili in ogni piano. Fare clic sul nome di una funzionalità per leggere un breve riepilogo del funzionamento in Yammer.
  
|**Funzionalità**|**Yammer Basic**|**Yammer Enterprise**|
|:-----|:-----|:-----|
|[Gruppi](group-features-in-yammer.md#groups) <br/> | Sì  <br/> |Sì  <br/> |
|[Gruppi correlati](group-features-in-yammer.md#related-groups) <br/> |Sì  <br/> |Sì  <br/> |
|[Annunci](group-features-in-yammer.md#announcements) <br/> |No  <br/> |Sì  <br/> |
|[Profili utente](profile-features-in-yammer.md#user-profiles) <br/> |Sì  <br/> |Sì  <br/> |
|[Expertise](profile-features-in-yammer.md#expertise) <br/> |Sì  <br/> |Sì  <br/> |
|[Elenco iscritti](profile-features-in-yammer.md#member-directory) <br/> |Sì  <br/> |Sì  <br/> |
|[Organigramma](profile-features-in-yammer.md#org-chart) <br/> |Sì  <br/> |Sì  <br/> |
|[Complimenti](profile-features-in-yammer.md#praise) <br/> |Sì  <br/> |Sì  <br/> |
|[Classifica ufficiale](profile-features-in-yammer.md#leaderboards) <br/> |Sì  <br/> |Sì  <br/> |
|[Server Editore](message-and-conversation-features-in-yammer.md#publisher) <br/> |Sì  <br/> |Sì  <br/> |
|[@Menzioni](message-and-conversation-features-in-yammer.md#section) <br/> |Sì  <br/> |Sì  <br/> |
|[Online Now](message-and-conversation-features-in-yammer.md#online-now) <br/> |Sì  <br/> |Sì  <br/> |
|[Messaggi privati](message-and-conversation-features-in-yammer.md#private-messages) <br/> |Sì  <br/> |Sì  <br/> |
|[Condividi conversazioni](message-and-conversation-features-in-yammer.md#share-conversations) <br/> |Sì  <br/> |Sì  <br/> |
|[Condividi conversazioni](message-and-conversation-features-in-yammer.md#share-conversations) <br/> |Sì  <br/> |Sì  <br/> |
|[Eventi](message-and-conversation-features-in-yammer.md#events) <br/> |Sì  <br/> |Sì  <br/> |
|[Sondaggi](message-and-conversation-features-in-yammer.md#polls) <br/> |Sì  <br/> |Sì  <br/> |
|[File](document-collaboration-features-in-yammer.md#files) <br/> |Sì  <br/> |Sì  <br/> |
|[Elenco delle modifiche recenti](document-collaboration-features-in-yammer.md#recent-changes) <br/> |Sì  <br/> |Sì  <br/> |
|[Contenuto ufficiale](document-collaboration-features-in-yammer.md#official-content) <br/> |No  <br/> |Sì  <br/> |
|[Posta in arrivo](inbox-features-in-yammer.md#inbox) <br/> |Sì  <br/> |Sì  <br/> |
|[Ricerca Posta in arrivo](inbox-features-in-yammer.md#inbox-search) <br/> |Sì  <br/> |Sì  <br/> |
|[Inserimenti](discovery-features-in-yammer.md#feeds) <br/> |Sì  <br/> |Sì  <br/> |
|[Ticker](discovery-features-in-yammer.md#ticker) <br/> |Sì  <br/> |Sì  <br/> |
|[Argomenti](discovery-features-in-yammer.md#topics) <br/> |Sì  <br/> |Sì  <br/> |
|[Ricerca universale](discovery-features-in-yammer.md#universal-search) <br/> |No  <br/> |Sì  <br/> |
|[Completamento istantaneo](discovery-features-in-yammer.md#instant-type-ahead) <br/> |Sì  <br/> |Sì  <br/> |
|[Reti esterne](external-network-features-in-yammer.md#external-networks) <br/> |Sì  <br/> |Sì  <br/> |
|[Commutazione rete veloce](external-network-features-in-yammer.md#fast-network-switching) <br/> |Sì  <br/> |Sì  <br/> |
|[Accesso alla connessione Yammer](yammer-platform-features.md#yammer-connect-login) <br/> |Sì  <br/> |Sì  <br/> |
|[Feed incorporabili](yammer-platform-features.md#embeddable-feeds) <br/> |Sì  <br/> |Sì  <br/> |
|[Pulsanti Mi piace e Segui](yammer-platform-features.md#like-and-follow-buttons) <br/> |Sì  <br/> |Sì  <br/> |
|[Estensioni browser](yammer-platform-features.md#browser-extension) <br/> |Sì  <br/> |Sì  <br/> |
|[API flusso attività](yammer-platform-features.md#activity-stream-api) <br/> |Sì  <br/> |Sì  <br/> |
|[Custom Network Branding](administration-and-security-features-in-yammer.md#custom-network-branding) <br/> |No  <br/> |Sì  <br/> |
|[Gestione degli utenti](administration-and-security-features-in-yammer.md#user-management) <br/> |No  <br/> |Sì  <br/> |
|[Identità unica](administration-and-security-features-in-yammer.md#single-identity) <br/> |Sì  <br/> |Sì  <br/> |
|[Sincronizzazione delle directory](administration-and-security-features-in-yammer.md#directory-synchronization) <br/> |No  <br/> |Sì  <br/> |
|[Esporta dati](administration-and-security-features-in-yammer.md#data-export) <br/> |No  <br/> |Sì  <br/> |
|[Monitoraggio Parola chiave](administration-and-security-features-in-yammer.md#keyword-monitoring) <br/> |No  <br/> |Sì  <br/> |
|[Analisi](administration-and-security-features-in-yammer.md#analytics) <br/> |No  <br/> |Sì  <br/> |
|[Risorse della società](administration-and-security-features-in-yammer.md#company-resources) <br/> |Sì  <br/> |Sì  <br/> |
|[Criteri password](administration-and-security-features-in-yammer.md#password-policies) <br/> |No  <br/> |Sì  <br/> |
|[Gestione delle sessioni](administration-and-security-features-in-yammer.md#session-management) <br/> |No  <br/> |Sì  <br/> |
|[Logical Firewall](administration-and-security-features-in-yammer.md#logical-firewall) <br/> |No  <br/> |Sì  <br/> |
|[Criteri di utilizzo](administration-and-security-features-in-yammer.md#usage-policy) <br/> |No  <br/> |Sì  <br/> |
|[FastTrack Center](http://go.microsoft.com/fwlink/?LinkID=518597&amp;clcid=0x409) <br/> |No  <br/> |Sì  <br/> |
|[Assistenza clienti](support-features-in-yammer.md#customer-support) <br/> |Sì  <br/> |Sì  <br/> |
|[Supporto per amministratori](support-features-in-yammer.md#administrator-support) <br/> |No  <br/> |Sì  <br/> |
|[Admin Private Content Mode](administration-and-security-features-in-yammer.md#admin-private-content-mode) <br/> |No  <br/> |Sì  <br/> |
   

