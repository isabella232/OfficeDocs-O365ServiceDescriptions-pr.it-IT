...
Per crittografare la connessione tra server di posta elettronica, in modo da evitare lo spoofing e offrire la necessaria riservatezza per i messaggi in transito, viene utilizzato TLS (Transport Layer Security). TLS viene utilizzato anche per proteggere il traffico del server di posta locale verso i datacenter Microsoft per Archiviazione Exchange Online.
...
- SSL viene utilizzato per proteggere Outlook, Outlook sul web e Exchange Web Services, utilizzando la porta TCP 443.
...
Gli utenti possono leggere e creare messaggi protetti tramite IRM in modo nativo in Outlook sul web, proprio come in Outlook. I messaggi protetti con IRM in Outlook sul web possono essere accessibili tramite Internet Explorer, Firefox, Safari e Chrome (senza alcun plug-in necessario). Per i messaggi sono disponibili la ricerca di testo, la visualizzazione delle conversazioni e il riquadro di anteprima. Per abilitare questa funzionalità è necessario che sia configurata l'interoperabilità tra il server AD RMS e l'ambiente locale di Exchange.
